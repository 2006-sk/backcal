...
class OpenAIHelper:
    ...
    def _create_session(self):
        ...
        # Configure retry strategy (more aggressive for SSL errors)
...
Be concise. Return only valid JSON."""
    ...
    def _analyze_with_openai_gpt4o(self, video_path: Path, frames: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            # Analyze ALL frames for comprehensive insights
            frames_to_analyze = frames
            
            print(f"[Vision] Analyzing {len(frames_to_analyze)} frames with OpenAI GPT-4o-mini")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"[Vision] OpenAI API call failed (attempt {attempt+1}/{max_retries}): {response.status_code}")
                        if attempt < max_retries - 1:
                            import time
                            time.sleep(2 ** attempt)  # Exponential backoff
                        
                except Exception as e:
                    print(f"[Vision] OpenAI API error (attempt {attempt+1}/{max_retries}): {e}")
                    if attempt < max_retries - 1:
                        import time
                        time.sleep(2 ** attempt)  # Exponential backoff
                    else:
